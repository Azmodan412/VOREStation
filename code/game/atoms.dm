...
/atom/movable/New()
	...
	if(global.use_preloader && (src.type == global._preloader.target_path))
		global._preloader.load(src)

	// Pass our arguments to InitAtom so they can be passed to initialize(), but replace loc with if we're in mapload.
